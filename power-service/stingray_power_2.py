...
class Ltc2992:
  _dev_name='ltc2992'
  _gpio_name=''
  _gpiochips = []  # Store the gpiochips corresponding to ltc2992

  def __init__(self, stingray_device_index=0):
    self.find_gpiochips(stingray_device_index)

  def find_gpiochips(self, stingray_device_index=0):
      """Find and store the gpiochips corresponding to ltc2992 devices."""
      devices = os.popen('grep "" /sys/class/gpio/gpiochip*/device/name').read()
      devices = devices.split(sep="\n")
      count = sum(1 for line in devices if self._dev_name in line)
      print(f"[DEBUG] Number of devices with name '{self._dev_name}': {count}")

      # Clear the _gpiochips list and initialize with None
      self._gpiochips = [None, None]

      # Handle cases based on the count of devices
      if count == 1:
          for line in devices:
              if self._dev_name in line:
                  device = line.split(sep="/")
                  for part in device:
                      if 'gpiochip' in part:
                          gpiochip = part.replace('gpiochip', '')
                          self._gpiochips[stingray_device_index] = gpiochip
                          print(f"[DEBUG] Assigned gpiochip: {gpiochip} to index {stingray_device_index}")
                          break
      elif count == 2:
          index = 0
          for line in devices:
              if self._dev_name in line:
                  ...
                  for part in device:
                      if 'gpiochip' in part:
                          gpiochip = part.replace('gpiochip', '')
                          self._gpiochips[index] = gpiochip
                          print(f"[DEBUG] Assigned gpiochip: {gpiochip} to index {index}")
                          index += 1
                          break
      else:
          print("[ERROR] Unexpected number of devices found")

      # Handle cases where not all GPIO chips are found
      if self._gpiochips[0] is None:
          print("[ERROR] GPIO chip for index 0 not found")
      if self._gpiochips[1] is None:
          if stingray_device_index == 1:
              raise ValueError("[ERROR] GPIO chip for index 1 not found, but index 1 was requested")
          else:
              print("[WARNING] GPIO chip for index 1 not found")

      print(f"[DEBUG] Final GPIO chips list: {self._gpiochips}")

  def export_gpios(self, gpiochip):
      """Export GPIOs for a given gpiochip, set the GPIO name, and configure the direction."""
      gpio_base = int(gpiochip, 10)
      print(f"[DEBUG] Found gpiochip base: {gpio_base} for device {self._dev_name}")
      
      # Export exactly 4 GPIOs for the device
      for offset in range(4):
          gpio = gpio_base + offset
          print(f"[DEBUG] Attempting to export GPIO: {gpio}")  # Debug print for each GPIO
          result = os.popen(f'echo {gpio} > /sys/class/gpio/export').read()
          print(f"[DEBUG] Log for GPIO {gpio} after export: {result.strip()}")  # Debug print for export result

      # Set the GPIO name based on the exported GPIOs
      devices = os.popen('ls /sys/class/gpio').read()
      devices = devices.split(sep="\n")
      print(f"[DEBUG] Devices in /sys/class/gpio: {devices}")  # Debug print for devices list

      for line in devices:
          if f"ltc2992-6a-GPIO" in line:
              self._gpio_name = line.replace('ltc2992-6a-GPIO1', 'ltc2992-6a-GPIO')
              # print(f"[DEBUG] Matched GPIO name: {self._gpio_name}")  # Debug print for matched GPIO name
              break

      if not self._gpio_name:
          print(f"[ERROR] GPIO name not found for gpiochip base: {gpio_base}")  # Debug print for error
          raise SystemError('Stingray power: Cant get GPIOs')

      # print(f"[DEBUG] GPIO name set to: {self._gpio_name}")

  def unexport_gpios(self, gpiochip):
      """Unexport GPIOs for a given gpiochip."""
      gpio_base = int(gpiochip, 10)
      print(f"[DEBUG] Found gpiochip base: {gpio_base} for device {self._dev_name}")
      
      # Unexport exactly 4 GPIOs for the device
      for offset in range(4):
          gpio = gpio_base + offset
          print(f"[DEBUG] Unexporting GPIO: {gpio}")  # Debug print for each GPIO
          os.popen(f'echo {gpio} > /sys/class/gpio/unexport').read()
...
class GPIOS:
  ...
  def get_device(self, dev_label):
      """Get the device name based on the label."""
      devices = os.popen('grep "" /sys/bus/iio/devices/iio\:device*/label').read()
      devices = devices.split(sep="\n")

      print(f"[DEBUG] Devices found for label search: {devices}")  # Debug print

      # Initialize the device variable
      device = None

      # Search for the device label
      for line in devices:
      ...
      if device is None:
          raise ValueError(f"Device with label '{dev_label}' not found.")
      ...
      print(f"[DEBUG] Matched device: {device}")  # Debug print
      return device

  def gpio_pulse(self, iio, ch, powerup=True, index=0):
      # Determine the device label based on the index
      if index == 0:
          dev_label = 'stingray0_control'
      elif index == 1:
          dev_label = 'stingray1_control'
      else:
          raise ValueError(f"Invalid index: {index}. Must be 0 or 1.")

      # Get the device name based on the label
      device = self.get_device(dev_label=dev_label)

      # Pulse the GPIO pin
      print(f"[DEBUG] Pulsing GPIO for device: {device}, channel: {ch}, powerup: {powerup}")
...
class Stingray:
  ...
  def __init__(self, stingray_device_index=0):
    self._iio = IIO()
    self._ltc = Ltc2992(stingray_device_index)
    self._gpio = GPIOS()
  ...
  def pulse_power_pin(self, which, powerup=True, index=0):
      if index < 0 or index >= len(self._ltc._gpiochips):
          raise IndexError(f"Invalid index: {index}. Available indices: 0 to {len(self._ltc._gpiochips) - 1}")

      # Determine the GPIO pin based on the device index
      if which.lower() == 'pwr_up_down':
          gpio_pin = self._gpio.PWR_UP_DOWN_PIN
      elif which.lower() == '5v_ctrl':
          gpio_pin = self._gpio.P5V_CTRL_PIN
      else:
          raise ValueError(f"Invalid pin: {which}. Must be 'pwr_up_down' or '5v_ctrl'.")

      # Pulse the GPIO pin
      self._gpio.gpio_pulse(self._iio, gpio_pin, powerup, index=index)
      print(f"[DEBUG] Pulsing GPIO pin {gpio_pin} for stingray {index}, powerup: {powerup}")
  ...
  def powerdown(self):
    self._devices = []  # Clear the devices list
    # Get the devices
    self.get_devices()
    ...
    if self.fully_powered():
      ...
      self.pulse_power_pin('5v_ctrl', False, index=0) #todo: change this!!!
    ...
    if self.partially_powered():
      ...
      self.pulse_power_pin('pwr_up_down', False, index=0) #todo: change this!!!
  ...
  def bind_devices(self, index=0):
      """
      Bind SPI devices based on the given index.

      Args:
          index (int): The index to determine which SPI devices to bind.
                      If 0, use spi1.x. If 1, use spi2.x.
      """
      if index == 0:
          spi_prefix = "spi1"
      elif index == 1:
          ...
          raise ValueError(f"Invalid index: {index}. Only 0 or 1 are allowed.")

      for i in range(1, 5):  # Loop through spi1.1 to spi1.4 or spi2.1 to spi2.4
          os.popen(f'echo -n "{spi_prefix}.{i}" > /sys/bus/spi/drivers/adar1000/unbind').read()
          os.popen(f'echo -n "{spi_prefix}.{i}" > /sys/bus/spi/drivers/adar1000/bind').read()

      os.popen('systemctl restart iiod').read()

  def powerup(self, stingray_device_index=0, enable_5v=True, **kwargs):
      """
      Power up the Stingray board.

      Args:
          stingray_device_index (int): Index to determine which stingray borad to use.
                      If 0, use gpiochip[0] and spi1.x for stingray board 0.
                      If 1, use gpiochip[1] and spi2.x for stingray board 1.
          enable_5v (bool): Whether to enable the 5V rail.
      ...
      # Validate the index
      if stingray_device_index < 0 or stingray_device_index >= len(self._ltc._gpiochips):
          raise IndexError(f"Invalid index: {stingray_device_index}. Available indices: 0 to {len(self._ltc._gpiochips) - 1}")

      # Export GPIOs for the specified gpiochip
      self._ltc.export_gpios(self._ltc._gpiochips[stingray_device_index])
      ...
      if not self.partially_powered():
          self.pulse_power_pin('pwr_up_down', True, index=stingray_device_index)
          ...
          # Bind SPI devices for the specified index
          self.bind_devices(stingray_device_index)
      ...
      if len(self._devices) == 0:
          ...
          print('[OKAY] Stingray power: Found ' + str(len(self._devices)) + ' ADAR1000 devices')
      ...
      if enable_5v and not self.fully_powered():
          self.pulse_power_pin('5v_ctrl', True, index=stingray_device_index)

          if not self.fully_powered():
              print("[FAILED] Stingray power: Power up sequence failed")
          else:
              print("[OKAY] Stingray power: Power up sequence succeeded")
      # Unexport GPIOs for the specified gpiochip
      self._ltc.unexport_gpios(self._ltc._gpiochips[stingray_device_index])

if __name__ == '__main__':
  try:
    if sys.argv[1] == 'up':
      # print("Stingray power: Start power up sequence")
      # Power up the first board stingray_0
      print(f"[DEBUG] Powering up stingray_0 with index 0")
      stingray_0 = Stingray(stingray_device_index=0)
      try:
          stingray_0.powerup(stingray_device_index=0, enable_5v=True)
      except Exception as e:
          print(f"[ERROR] Exception in powerup stingray_0: {e}")
          raise
      # Power up the second board stingray_1
      print(f"[DEBUG] Powering up stingray_1 with index 1")
      stingray_1 = Stingray(stingray_device_index=1)
      try:
          stingray_1.powerup(stingray_device_index=1, enable_5v=True)
      except Exception as e:
          print(f"[ERROR] Exception in powerup stingray_1: {e}")
          raise
      stingray_1.powerup(stingray_device_index=1, enable_5v=True)
