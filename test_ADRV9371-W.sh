...
rsync -azr -e "ssh -i /root/.ssh/id_rsa" /root/osc-logs test_results@romlx1:~/AD9371-W &
...
if [[ -f /root/osc-logs/AD9371-W-"${serial}"-00.tar.gz ]] ; then
    num=$(ls -l /root/osc-logs/AD9371-W-${serial}* | sort -g | sed -n 1p | awk -F "-" '{print $NF}' | awk -F "." '{print $1}')
...
tar -czf /root/osc-logs/AD9371-W-"${serial}".tar.gz osc_run.txt markers.log ADRV*.png
